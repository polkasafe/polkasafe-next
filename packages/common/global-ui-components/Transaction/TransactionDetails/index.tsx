...
export default function TransactionDetails({ type, callHash, callData, createdAt, amountToken, to, network, from, transactionType, onAction, approvals, signatories, threshold, hasApproved  }: ITransactionDetails) {
	return <div className="p-4 bg-bg-secondary flex items-start gap-x-4">
			...
			<div className="rounded-xl p-4 bg-bg-main basis-1/3">
						...
						>
							<div className='text-white font-normal text-sm leading-[15px]'>
								...
								<span className='text-text_secondary'>
									{transactionType === ETransactionType.HISTORY_TRANSACTION ? threshold : approvals?.length} of {threshold}
								</span>
							</div>
						</Timeline.Item>
						<Timeline.Item
				...
				{transactionType === ETransactionType.QUEUE_TRANSACTION && 
					<div className="mt-2 flex flex-col gap-y-4 px-2">
						{!hasApproved && 
						<Button onClick={() => onAction(ETxType.APPROVE)} fullWidth icon={<OutlineCheckIcon className="text-success" />} className="bg-[#06d6a0]/[0.1] text-success border-none">Approve</Button>}
						<Button onClick={() => onAction(ETxType.CANCEL)} icon={<OutlineCloseIcon className="text-failure" />} fullWidth variant={EButtonVariant.DANGER}>Reject</Button>
					</div>
				}
			</div>
		}
