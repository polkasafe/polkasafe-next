...
import SetIdentity from '@common/modals/NewTransaction/components/NewTransactionForm/SetIdentity';
...
export function NewTransactionForm({
	...
	const handleSubmit = async () => {
		try {
			...
			const identityData = form.getFieldValue('identityData');
			...
			if (checkRecipent.length && type === ETransactionCreationType.SEND_TOKEN) {
			...
			setLoading(true);
			await buildTransaction({ ...payload, identityData });
		} catch (error) {
				...
				>
					{type === ETransactionCreationType.SET_IDENTITY ? (
						<div className='flex flex-col gap-y-6'>
							...
							<SetIdentity
								multisigAddress={selectedMultisigDetails.address}
								form={form}
							/>
						</div>
					) : (
