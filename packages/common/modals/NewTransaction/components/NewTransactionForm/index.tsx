...
import ReviewTransaction from '@common/modals/NewTransaction/components/NewTransactionForm/ReviewTransaction';
...
export function NewTransactionForm({
	...
	const { multisigs, onNewTransaction, addressBook, currencyValues, allApi } = useDashboardContext();
	...
	useEffect(() => {
		if (
			!allApi ||
			!selectedMultisigDetails ||
			!allApi[selectedMultisigDetails.network] ||
			!allApi[selectedMultisigDetails.network].apiReady ||
			!recipientAndAmount ||
			recipientAndAmount.some((item) => item.recipient === '' || item.amount.isZero())
		)
			return;

		const { network } = selectedMultisigDetails;
		...
		if (selectedMultisigDetails.proxy) {
			tx = allApi[network].api.tx.proxy.proxy(selectedMultisigDetails.proxy, null, batch);
			setCallData(tx.method.toHex());
		} else {
			setCallData(batch.method.toHex());
		}
	}, [allApi, recipientAndAmount, selectedMultisigDetails]);
			...
			>
				...
				>
					{step === ETransactionSteps.BUILD_TRANSACTION ? (
						...
						<ReviewTransaction
							callData={callData}
							api={allApi[selectedMultisigDetails.network].api}
							from={selectedMultisigDetails.address}
							to={recipientAndAmount[0].recipient}
							network={selectedMultisigDetails.network}
						/>
					)}
